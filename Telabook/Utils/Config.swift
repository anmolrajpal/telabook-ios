...
struct Config {
    ...
    static var environment:Environment {
        do {
            return .init(rawValue: try Configuration.value(for: .environment))
        } catch {
            fatalError("\(error)")
        }
    }
    ...
    static var mixpanelToken:String {
        do {
            return try Configuration.value(for: .mixpanelToken)
        } catch {
            fatalError("\(error)")
        }
    }
    ...
    static let isTestFlightBuild = Bundle.main.appStoreReceiptURL?.lastPathComponent == "sandboxReceipt"
    ...
    static var appName:String {
        do {
            return try Configuration.value(for: .bundleDisplayName)
        } catch {
            fatalError("\(error)")
        }
    }
